...
func (a *AuthController) Login(c *fiber.Ctx) error {
	...
	user, modelBranch, err := a.AuthService.Login(c, req)
	if err != nil {
		return err
	}
	...
	var activeBranch *response.ActiveBranchResponse
	if modelBranch != nil {
		activeBranch = &response.ActiveBranchResponse{
			ID:   modelBranch.ID.String(),
...
func (a *AuthController) GetActiveBranch(c *fiber.Ctx) error {
    userID := c.Locals("user_id").(string)

    active, err := a.AuthService.GetActiveBranch(c, userID)
    if err != nil {
        return err
    }

...
import { deleteRecipeItem } from "@/services/recipeService";
import toast from "react-hot-toast";

interface DeleteRecipeItemModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function DeleteRecipeItemModal({ isOpen, onClose, recipe, recipeItem, onSuccess }: DeleteRecipeItemModalProps) {
  const [loading, setLoading] = useState(false);

  const handleDelete = async () => {
    if (!recipe || !recipeItem) return;

    setLoading(true);
    try {
      const response = await deleteRecipeItem(recipe.id!, recipeItem.item_id);
      
      if (response.isSuccess) {
        toast.success(response.message || "Recipe item deleted successfully");
        onSuccess?.();
        onClose();
      } else {
        toast.error(response.message || "Failed to delete recipe item");
      }
    } catch (error) {
  ...
  if (!isOpen || !recipe || !recipeItem) return null;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-900">Delete Recipe Item</h3>
          ...
          <div className="flex items-center mb-4">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-gray-500">
                <p>
                  This action will remove the item <strong>ID: {recipeItem.item_id}</strong> from recipe <strong>{recipe.name}</strong>.
                </p>

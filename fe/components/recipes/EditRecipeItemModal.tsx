...
import { updateRecipeItem } from "@/services/recipeService";
...
export default function EditRecipeItemModal({ isOpen, onClose, recipe, recipeItem, onSuccess }: EditRecipeItemModalProps) {
  const [loading, setLoading] = useState(false);
  const [quantity, setQuantity] = useState("");

  useEffect(() => {
    if (recipeItem) {
      setQuantity(recipeItem.quantity?.toString() || "");
    }
  }, [recipeItem]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const updateData: Partial<RecipeItemType> = {
        quantity: parseFloat(quantity),
      };

      const response = await updateRecipeItem(recipe!.id!, recipeItem!.item_id, updateData);
      
      if (response.isSuccess) {
        toast.success(response.message || "Recipe item updated successfully");
        onSuccess?.();
        handleClose();
      } else {
        toast.error(response.message || "Failed to update recipe item");
      }
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-900">Edit Recipe Item</h3>

...
interface ImportSalesFromIsellerProps {
  isOpen: boolean;
  onClose: () => void;
  onImport: (items: File, forceImport: boolean) => void;
  loading?: boolean;
...
export default function ImportSalesFromIseller({ isOpen, onClose, onImport, loading, notFoundProductCodes }: ImportSalesFromIsellerProps) {
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center p-4">
      <div className="relative bg-white rounded-lg shadow-xl max-w-2xl w-full mx-auto">
        ...
        <div className="flex items-center justify-end space-x-3 px-6 py-4 border-t border-gray-200">
          ...
          {selectedFile && (
            <button
              onClick={() => onImport(selectedFile, !!(notFoundProductCodes && notFoundProductCodes.length > 0))}
              disabled={loading}
              className={`px-4 py-2 text-sm font-medium text-white border border-transparent rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center ${
                notFoundProductCodes && notFoundProductCodes.length > 0
